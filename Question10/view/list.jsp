			...
			<td style="width: 100px;">
				<% if(list.get(i).getTf().equals("送信済")){ %>
					<a href="<%=request.getContextPath()%>/detail?num=<%=qObj.getNum()%>">
						<input type="submit" value=<%=qObj.getTf()%> style="color:dimgray; background-color:sandybrown;">
					</a>
				<% } else{ %>
					<a href="<%=request.getContextPath()%>/detail?num=<%=qObj.getNum()%>">
						<input type="submit" value=<%=qObj.getTf()%> style="color:darkred; background-color:ivory;">
					</a>
				<% } %>
			</td>
		</tr>
